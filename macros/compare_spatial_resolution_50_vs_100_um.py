...
for d, dnames in enumerate(datasets):
    ...
    #varname = 'amplitude_vs_x_channel0'
    #plotfile_name = 'AmpPlots/PlotAmplitudeVsX.root'
    ...
    plotfile = TFile(plotfile_name_1)
    inputfile = TFile(inputfile_name_1)
    dataset = dnames[0]
    ...
    for i,ch in enumerate(channel_good_index):
        print(f"{varname}{i}")
        plot_amplitude = plotfile.Get(f"{varname}{i}")
        plot_amplitude.SetLineWidth(2)
        plot_amplitude.SetFillColor(colors[i])
        plot_amplitude.SetFillStyle(3050)
        plot_amplitude.SetFillColorAlpha(colors[i],0.3)
        plot_amplitude.SetLineColor(colors[i])
        plot_amplitude.SetMaximum(2)
        plotList_amplitude_vs_x.append(plot_amplitude)
        plot_amplitude2 = plotfile2.Get(f"{varname}{i}")
        plot_amplitude2.SetLineWidth(2)
        plot_amplitude2.SetLineColor(colors[i])
        plot_amplitude2.SetMaximum(2)
    ...
    totalAmplitude_vs_x.GetYaxis().SetTitle("Two strip reconstruction efficiency")
    totalAmplitude_vs_x.SetLineWidth(2)
    totalAmplitude_vs_x.SetMaximum(1.6)
    ...
    for i,ch in enumerate(channel_good_index):
        plotList_amplitude_vs_x[i].Draw("same f")
        plotList_amplitude_vs_x2[i].Draw("same")
