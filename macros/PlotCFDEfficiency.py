...
rebin = 2
efficiency_denominator_global.Rebin2D(rebin)
...
for i in range(7):
    if inputfile.Get("efficiency_vs_xy_fullReco_numerator_channel0%i"%(i)):
        ...
        list_efficiency_vs_xy_fullReco_numerator_ch[-1].Rebin2D(rebin)
        EfficiencyUtils.Plot2DEfficiency(list_efficiency_vs_xy_fullReco_numerator_ch[-1], efficiency_denominator_global, "%sEfficiencyFullReco_ch0%i"%(outdir,i), "", "Track x position [mm]", -xlength, xlength, "Track y position [mm]", -20, 20, 0.0, 1.0, True)
...
for n,name in enumerate(list_hist_coarse_bin):
    ...
    good_hist = EfficiencyUtils.Make1DEfficiencyHist(this_hist, this_denom, list_name_coarse_bin[n], list_name_coarse_bin[n], "X [mm]", -xlength, xlength, 0.0, 0.0)
