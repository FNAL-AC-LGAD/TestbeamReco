...
for l in range(len(projection)) : 
    ...
    totalAmplitude_vs_x.GetXaxis().SetTitle("Relative "+ axis_label +" position [mm]")
    totalAmplitude_vs_x.GetXaxis().SetRangeUser(-0.63, 0.63)
    ...
    if (l==0) :
        boxes = getStripBox(inputfile,ymin*1.1,ymax*1.04,False,18,False,centerShift)
    else : 
        boxes = getStripBoxY(inputfile,ymin*1.1,ymax*1.04,False,18,centerShift)
    for box in boxes:
       box.Draw() 
