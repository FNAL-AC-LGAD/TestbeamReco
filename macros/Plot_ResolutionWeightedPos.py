...
hTwoEff = inFile.Get("hefficiency_vs_x_twoStrip_numerator"+tight_postfix)
hOneEff = inFile.Get("hefficiency_vs_x_oneStrip_numerator"+tight_postfix)
...
for i in range(1, nbins+1):
    OneAndTwoStripPosRes = [0.0, 0.0]
    OneAndTwoStripPosResError = [0.0, 0.0]
    # Apply weights
    ...
    print("{:.2f} -> {:.3f} ({:.2f}), {:.3f} ({:.2f})".format(tmpHistOne.GetXaxis().GetBinCenter(i), oneEff, tmpHistOne.GetBinContent(i), twoEff, tmpHistTwo.GetBinContent(i)))
    if(oneEff+twoEff <= 0):
        value = 0
    else:
