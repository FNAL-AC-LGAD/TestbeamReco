...
for l in range(len(AmpRatio_list)) : 
    ...
    for i in range(0, AmpRatio.GetYaxis().GetNbins() + 1):
        ...
        if(nEntries > 10.0):
            myGausFunction = TF1("mygaus","gaus(0)",-3.5,3.5);
            tmpHist.Fit(myGausFunction,"Q","",-3.5,3.5);
    ...
    if (l==0) : 
        canvas.SaveAs(outdir+"PositionFitTop.gif")
        canvas.SaveAs(outdir+"PositionFitTop.pdf")
    elif (l==1)  :
        canvas.SaveAs(outdir+"PositionFitBot.gif")
        canvas.SaveAs(outdir+"PositionFitBot.pdf")
    elif (l==2) :
        canvas.SaveAs(outdir+"PositionFitRight.gif")
        canvas.SaveAs(outdir+"PositionFitRight.pdf")
    else :
        canvas.SaveAs(outdir+"PositionFitLeft.gif")
        canvas.SaveAs(outdir+"PositionFitLeft.pdf")
