...
sensor_list = ["EIC_W1_1cm_500up_200uw_215V","EIC_W1_1cm_500up_200uw_225V","EIC_W1_1cm_500up_200uw_235V","EIC_W1_1cm_500up_200uw_245V","EIC_W1_1cm_500up_200uw_255V","EIC_W1_1cm_500up_200uw_265V"]
list_input = []
...
for nSensor,item1 in enumerate(sensor_list):
    ...
    stripindex = ["01hot","01cold","01gap","01"];
    TH1DVar =  [];
    
    for nstrip,item2 in enumerate(stripindex):
        
        TH1DVar.append(list_input[nSensor].Get("risetime%s"%(stripindex[nstrip])))
        VarMean[nSensor][nstrip] = TH1DVar[nstrip].GetMean()
...
xValue = [215,225,235,245,255,265]
...
hdummy = ROOT.TH1D("","",1,210,270)
hdummy.GetXaxis().SetTitle("Bias voltage [V]")
hdummy.GetYaxis().SetTitle("Risetime [ps]")
hdummy.SetMaximum(850)
hdummy.SetMinimum(401)
hdummy.Draw("AXIS")
...
TopRightText.DrawLatexNDC(1-myStyle.GetMargin()-0.005,1-myStyle.GetMargin()+0.01,"#bf{BNL 10-200}")

myStyle.BeamInfo()

canvas.SaveAs(outdir+"RiseTime_DiffBias.gif")
canvas.SaveAs(outdir+"RiseTime_DiffBias.pdf")
