...
void InitialAnalyzer::InitHistos(NTupleReader&, const std::vector<std::vector<int>>&)
{
    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    ...
    utility::makeHisto(my_histos,"time", "; Time [ns]; Events", 1000,-0.5,1000.5);
...
void InitialAnalyzer::Loop(NTupleReader& tr, int maxevents)
{
    ...
    while( tr.getNextEvent() )
    {
        ...
        utility::fillHisto(pass, my_histos["time"], corrTime[photekIndex]);
    } //event loop
}
