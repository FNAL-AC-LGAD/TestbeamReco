...
void Analyze::InitHistos(NTupleReader& tr, const std::vector<std::vector<int>>& geometry)
{
    ...
    utility::makeHisto(my_2d_histos,"Amp1OverAmp1and2_vs_deltaXmax", "", stripWidth/0.002,pitch/2,stripWidth/2, 100,0.0,1.0);
...
void Analyze::Loop(NTupleReader& tr, int maxevents)
{
    ...
    while( tr.getNextEvent() )
    {
        ...
        const auto& maxAmpPadIndex = tr.getVar<int>("maxAmpPadIndex");
	...
	//******************************************************************        
        ...
        for(const auto& row : ampLGAD)
        {
            for(unsigned int i = 0; i < row.size(); i++)
            {
                ...
                utility::fillHisto(pass && goodHit && (maxAmpinPad1 || maxAmpinPad2),       my_histos["relFrac_bottom"+r+s], relFrac[LGAD_index]);
                utility::fillHisto(pass && goodHit && (maxAmpinPad3 || maxAmpinPad4),       my_histos["relFrac_top"+r+s], relFrac[LGAD_index]);
        ...
        utility::fillHisto(pass && goodMaxLGADAmp,                                     my_2d_histos["relFracMaxAmp_vs_x"], padx, relFrac[maxAmpPadIndex-1]);
