...
public:
    ...
    double pitch = 0.500;
    double sensorCenter  = 2.0;   
    double sensorCenterY = 0.0   ; 
    std::vector<double> stripCenterXPosition = {2.25, 2.25, 1.75, 1.75, 1.25, 1.25, 2.0, 2.0};
    int numLGADchannels = 6;
    int lowGoodStripIndex = 2; // Based on the strip centerX index -> picking middle strip analog readout
    ...
    double xmin = 0.5; // Sensor's local frame
    double xmax = 3.5; // Sensor's local frame
    ...
    std::vector<std::vector<double>> sensorEdges = {{0.5, -4.5}, {3.5, 4.5}}; // Sensor's local frame
